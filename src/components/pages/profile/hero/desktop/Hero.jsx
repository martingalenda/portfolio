...
const Hero = () => {
    ...
    // Efecto para ocultar/mostrar el logo__small
    useEffect(() => {
      const logoSmallElement = document.querySelector(".logo__small");
      if (logoSmallElement) {
        logoSmallElement.style.opacity = "0";
      }

      return () => {
        if (logoSmallElement) {
          logoSmallElement.style.opacity = "100";
        }
      };
    }, []);
            ...
            <div className="desktop_section">
                <section className="home hero">   
                    ...
                    className={`${wowActive ? 'wow animate__fadeIn' : ''}`} data-wow-duration="3s">
                        <CircularTxt text="- React UI Developer - PL ( APM / PO )" /> 
